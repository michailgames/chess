...
public class BoardPanel extends JPanel {
    ...
    private void paintLabels(Graphics g) {
        ...
        for (int i = 1; i <= Board.BOARD_SIZE; i++) {
            ...
            int fieldSize = y2 - y1;
            g.drawString(Integer.toString(9 - i), 5, y1 + (fieldSize + MARGIN_SIZE) / 2);
            g.drawString(Integer.toString(9 - i), calculateOffset(Board.BOARD_SIZE) + 5, y1 + (fieldSize + MARGIN_SIZE)
                    / 2);
            g.drawString(rankLabels[i - 1], y1 + fieldSize / 2 - 2, MARGIN_SIZE - 5);
            g.drawString(rankLabels[i - 1], y1 + fieldSize / 2 - 2, calculateOffset(Board.BOARD_SIZE) + MARGIN_SIZE - 5);
        }
    }
    ...
    private Color getDisplayColor(chess.model.board.Color color) {
        return (color == chess.model.board.Color.WHITE) ? WHITE_PIECES_COLOR : BLACK_PIECES_COLOR;
    }
    ...
    private int calculateOffset(int index) {
        return MARGIN_SIZE + (effectivePaintSize - 2 * MARGIN_SIZE) * index / Board.BOARD_SIZE;
    }
    ...
    private class BoardClickListener implements MouseListener {
        ...
        public void mousePressed(MouseEvent e) {
            ...
            int y = -1;
            for (int i = 0; i < Board.BOARD_SIZE; i++) {
                if (calculateOffset(i) <= e.getX() && e.getX() < calculateOffset(i + 1)) {
                    x = i;
                }
                if (calculateOffset(i) <= e.getY() && e.getY() < calculateOffset(i + 1)) {
                    y = i;
                }
            }
            if (x != -1 && y != -1) {
