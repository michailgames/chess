...
public class MinimaxPlayer extends AbstractAIPlayer {
    ...
    public MinimaxPlayer(Color color, IBoardEvaluationStrategy boardEvaluationStrategy, int depth) {
        super(color);
    ...
    private EvaluatedMove max(Board board, int depth, List<Move> availableMoves) {
        Move bestMove = null;
        ...
        for (Move move : availableMoves) {
            ...
            if (depth == 1 || !MoveUtils.hasAnyLegalMove(newBoard, getColor().getOppositeColor())) {
                score = evaluationStrategy.evaluateBoard(newBoard, getColor().getOppositeColor());
            } else {
                score = min(newBoard, depth - 1, MoveUtils.allLegalMoves(newBoard, getColor().getOppositeColor()))
                        .getScore();
            }
            if (score > bestScore) {
                bestMove = move;
    ...
    private EvaluatedMove min(Board board, int depth, List<Move> availableMoves) {
        Move worstMove = null;
        ...
        for (Move move : availableMoves) {
            ...
            if (depth == 1 || !MoveUtils.hasAnyLegalMove(newBoard, getColor())) {
                ...
                score = max(newBoard, depth - 1, MoveUtils.allLegalMoves(newBoard, getColor())).getScore();
            }
            if (score < worstScore) {
                worstMove = move;
                worstScore = score;
