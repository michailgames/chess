...
public class StandardBoardEvaluationStrategy implements IBoardEvaluationStrategy {
    ...
    public int evaluateBoard(Board board, Color nextPlayerColor) {
        ...
        if (gameState == GameState.MATE) {
            return nextPlayerColor == representedColor ? Integer.MIN_VALUE + 1000 : Integer.MAX_VALUE - 1000;
    ...
    public int getPieceValue(Piece piece, int x, int y) {
        if (piece instanceof Pawn) {
            if (piece.getColor() == Color.WHITE && y == 2 || piece.getColor() == Color.BLACK && y == 5) {
                return 110;
            }
            if (piece.getColor() == Color.WHITE && y == 1 || piece.getColor() == Color.BLACK && y == 6) {
                return 130;
            }
            return 100;
