...
/**
 * Projekt: Szachy
 * Test kontrolera logowania przebiegu gry
 * Micha³ Rapacz
 * 2015-05-17
 */
...
public class LogControllerTest {
	...
	public void moveIsLoggedProperly() {
		...
		GameController.getInstance().reportNewMove(whitePlayer, new Field(6, 6),
				new Field(6, 4));
		assertEquals(1, LogController.getInstance().getLogsNumber());
		String moveLog = LogController.getInstance().getLog(0);
		assertTrue(moveLog.contains(Pawn.getInstance(Color.WHITE)
				.getUnicodeString()));
	...
	public void oneMoveCausesOneLog() {
		...
		GameController.getInstance().reportNewMove(whitePlayer, new Field(6, 6),
				new Field(6, 4));
		assertEquals(1, LogController.getInstance().getLogsNumber());
		GameController.getInstance().reportNewMove(blackPlayer, new Field(6, 0),
				new Field(7, 2));
		assertEquals(2, LogController.getInstance().getLogsNumber());
		GameController.getInstance().reportNewMove(whitePlayer, new Field(5, 7),
				new Field(7, 5));
